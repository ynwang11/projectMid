...
(function () {
    ...
    function loadHistory(range) {
        ...
        return fetch('/api/history-data?range=' + encodeURIComponent(currentRange), { credentials: 'same-origin' })
            .then((r) => r.json())
            .then((data) => {
                if (data && data.error === 'subscription_required') {
                    if (fairyText) {
                        typePetals('要查看一月/三月/一年等长期报告，需要开通订阅。你仍可先查看「一周」趋势。');
                    }
                    return data;
                }
    ...
    // 范围 tabs
    root.querySelectorAll('.ag-range-tabs .time-tab').forEach((btn) => {
        btn.addEventListener('click', () => {
            const range = btn.getAttribute('data-range') || 'week';
            root.querySelectorAll('.ag-range-tabs .time-tab').forEach((b) => b.classList.remove('is-active'));
    ...
    // 默认激活一周
    root.querySelectorAll('.ag-range-tabs .time-tab').forEach((b) => b.classList.remove('is-active'));
    const defaultBtn = root.querySelector('.ag-range-tabs .time-tab[data-range="week"]');
